...
export default function IndexPage() {
  ...
  useEffect(() => {
    ...
    const filteredByCategory = places.filter((place) => selectedCategory === "" || place.category === selectedCategory);
    setNoExactMatches(filteredByCategory.length === 0);
  }, [filterPrice, places, selectedCategory]);
  ...
  return (
    <div className="">
      ...
      <div className="">
        <div className= "">
          <p className="text-black font-bold text-xl flex items-center justify-center mt-20">No Exact Matches</p>
          <p className="text-gray-500 flex items-center justify-center mt-2">Try changing or removing some of your filters.</p>
        </div>
        <div className="flex items-center justify-center mt-4">
          <button onClick={handleRefresh} className="bg-gray-100 flex hover:bg-gray-200 hover:text-white text-black py-3 px-8 rounded-md border border-black">
            Remove Filters
          </button>
        </div>
        {/* < Footer /> */}
      ...
      <div className="grid gap-x-4 gap-y-8 grid-cols-2 md:grid-cols-3 lg:grid-cols-4 mt-8 px-8">
        {(filterPrice ? filteredPlaces : places).filter((place) => selectedCategory === "" || place.category === selectedCategory).map((place) => (
          <Link to={"/place/" + place._id} key={place._id}>
            <div className="bg-gray-500 mb-2 rounded-2xl flex" style={{ height: "275px" , width: "292px" }}>
              {place.photos?.[0] && (
                <Image
